...
internal class NetworkData : ScriptableObject
{
	...
	private void OnValidate()
	{
		...
		if (m_playerPrefab != null)
		{
			...
			m_playerPrefab.GetComponent<NetworkObject>().NetID = 0;
	...
	public static int AddSceneObject(NetworkObject obj)
	{
		...
		if (obj.NetID > k_minSpawnedObjectIDCount && !Instance.m_unsavedDict.ContainsKey(obj.NetID))
		{
			Debug.Log($"Object ({GlobalObjectId.GetGlobalObjectIdSlow(obj)}) saved with an ID ({obj.NetID}) but the ID is not present in the dictionary. Most likely NetworkData wasn't saved.");
			Instance.m_unsavedDict.Add(obj.NetID, GlobalObjectId.GetGlobalObjectIdSlow(obj));
			SaveDict();
			return obj.NetID;
		}
	...
	public static bool HasSceneGameObject(NetworkObject obj)
	{
		...
		if (Instance.m_unsavedDict.TryGetValue(obj.NetID, out GlobalObjectId globalID))
		{
