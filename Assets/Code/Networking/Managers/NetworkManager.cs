...
public static class NetworkManager
{
	...
	public static void Init()
	{
		...
		var types = AppDomain.CurrentDomain.GetAssemblies()
			...
			.Where(p => typeof(Message).IsAssignableFrom(p));
	...
	internal static void ProcessMessage(SteamNetworkingMessage_t message, Peer sender)
	{
		...
		Message messageObj = (Message)generic.Invoke(null, new object[] { message.m_pData + Marshal.SizeOf(id) });

		// Filter message

		if (messageObj.Filter == Message.EMessageFilter.All ||
			messageObj.Filter == Message.EMessageFilter.ClientOnly && m_mode == ENetworkMode.Client ||
			messageObj.Filter == Message.EMessageFilter.HostOnly && m_mode == ENetworkMode.Host)
		{
	...
	public static void SendToServer<T>(T message) where T : Message
	{
		SendMessage(message, GetServerPeer());
	}

	public static void SendMessage<T>(T message, Peer target) where T : Message
	{
	...
	public static void BroadcastMessage<T>(T message) where T : Message
	{
		if (!SteamManager.Initialized)
			return;
	...
	public static Peer GetServerPeer()
	{
		if (m_mode == ENetworkMode.Host)
		{
			return null;
		...
		return m_localClient.m_hostPeer;
	}
