...
public static class NetworkObjectManager
{
	...
	internal static void AddNetworkObjectToList(NetworkObject networkObject)
	{
		...
		{
			m_persistentNetObjects.Add(networkObject.NetID, networkObject);
		}
		else
		{
			m_netObjects.Add(networkObject.NetID, networkObject);
		}
	}

	internal static void RemoveNetworkObjectFromList(NetworkObject networkObject)
	{
		if (networkObject.NetID < 0)
		{
			m_persistentNetObjects.Remove(networkObject.NetID);
		}
		else
		{
			m_netObjects.Remove(networkObject.NetID);
		}
	}
