		...
		Vector3 attackPosition, Vector3 attackDir, int baseDamage = 0, float damageMultiplier = 0, float baseRange = 0)
	{
		...
		switch (attackData.ApplicationType)
		{
			case EAttackApplicationType.Positional:
				...
				Physics.OverlapSphereNonAlloc(attackPosition, totalRange, colliders, attackData.AffectedLayers, QueryTriggerInteraction.Collide);
			...
			case EAttackApplicationType.Swinging:
				...
				// Store hit victems
				HashSet<IDamagable> hitVictims = new();
				...
				for (int i = 0; i < rayCount; i++)
				{
					...
					int hitCount = Physics.RaycastNonAlloc(attackPosition, rayDirection, hits, totalRange, attackData.AffectedLayers);
					for (int j = 0; j < hitCount; ++j)
					{
						if (hits[j].collider.TryGetComponent(out IDamagable damagable) && damagable.IsDamagable)
						{
							// Add the victim to the packet if not previously hit
							if (hitVictims.Add(damagable))
								packet.AddVictim(damagable, attackPosition);
						}
						else if (hits[j].collider.TryGetComponent(out Rigidbody rb)) // If the object has a rigidbody but no health, still apply knockback.
						{
							rb.AddForce((hits[j].collider.transform.position - attackPosition).normalized * packet.PacketKnockback, ForceMode.Impulse);
						}
					}
				}
				break;
